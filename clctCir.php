...
$sql =
  ...
  tt as (
      ...
      FROM
          ...
          INNER JOIN public.devices d ON (t.deviceid = d.id)   INNER JOIN public.longueur_circuit lc ON (cr.\"IDCIRCUIT\" = lc.id) 
          INNER JOIN PUBLIC.leplanning3 pl on pl.deviceid = d.id 
      where pl.datej = ?::date  --pl.datej >= current_date-1 and pl.datej <= current_date --and fonction = 'BALAYAGE MECANIQUE'
          and dd >= ?::timestamp 
          and df <= ?::timestamp
  ...
  -- order by circuit
  select distinct
    tt.circuit, tt.vehicule, tt.taux  , st_astext(routes.geom) as pl, tt.dd, tt.df, tt.fonction
  from 
    tt 
    join public.\"CIRCUIT_DET2\" crd on crd.idcircuit =  tt.idcircuit
    join public.routes on crd.idroutes = routes.ogc_fid
  where tt.id in(select deviceid from user_device where userid = ?) and taux != 0 and fonction like 'COLLECTE%'
  order by circuit desc
    ";
$prep = $bd->prepare($sql);
$ok = $prep->execute(array($_GET['dd'], $_GET['df'], $_GET['dd'], $_GET['dd'], $_GET['df'], $_GET['iduser']));

while ($d = $prep->fetch(PDO::FETCH_ASSOC)) {
  if ($d['fonction'] == 'COLLECTE BACS' or $d['fonction'] == 'COLLECTE COLLONES')
