...
$sql =
    "WITH t as ( 
        ...
        FROM
          gps.le_taux_tr3 tr3
        WHERE tr3.dh >= ?::timestamp AND tr3.dh <= ?::timestamp AND tr3.meth =1
        GROUP BY tr3.datej,tr3.id_cirdet,tr3.id_circuit,tr3.deviceid,tr3.lt,tr3.meth,tr3.pl
    ),
    tt as (
        select d.id,t.datej, cr.\"IDCIRCUIT\" as idcircuit,case when round(sum(t.ltr/(lc.lt*1000)*t.taux))  <=100 then round(sum(t.ltr/(lc.lt*1000)*t.taux)) else 100 end as taux,
    ...
    -- select tt.circuit, tt.vehicule, tt.taux , st_astext(crd.geom) as pl, tt.dd, tt.df
    ...
    ";
$prep = $bd->prepare($sql);
$ok = $prep->execute(array($_GET['dd'], $_GET['df'], $_GET['userid']));
